...
$(LIBDIR)/$(CONFIG)/libgrpc.a:
    $(E) "$(OPENSSL_DEP) $(ZLIB_DEP) $(CARES_DEP) $(ADDRESS_SORTING_DEP) $(RE2_DEP) $(UPB_DEP) $(GRPC_ABSEIL_DEP)  $(LIBGRPC_OBJS)  $(LIBGPR_OBJS)  $(LIBGRPC_ABSEIL_OBJS)  $(ZLIB_MERGE_OBJS)  $(CARES_MERGE_OBJS)  $(ADDRESS_SORTING_MERGE_OBJS)  $(RE2_MERGE_OBJS)  $(UPB_MERGE_OBJS)  $(OPENSSL_MERGE_OBJS)"
    $(E) "$(SYSTEM)"
	$(E) "[AR]      Creating $@"
	$(Q) mkdir -p `dirname $@`
